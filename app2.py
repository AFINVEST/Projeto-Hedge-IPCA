...
def process_df() -> pd.DataFrame:
    ...
    df_quant["Valor"] = df_quant["Valor"].astype(str).str.replace(",", ".").astype(float)
    ...
    df_merged["DIV1_ATIVO"] = (
        df_merged["Juros projetados"] * 0.0001 * (df_merged["Prazos (dias úteis)"] / 252)
    )
...
def obter_dap_dict_original() -> Dict[int, str]:
    return {
        ...
        2040: "DAP40", 2041: "DAP40", 2042: "DAP40", 2043: "DAP40", 2044: "DAP40", 2045: "DAP40",
    }
...
def check_duplicates(df: pd.DataFrame, label: str):
    ...
    if dups:
        st.warning(f"{label}: Encontradas {dups} linhas duplicadas de (Ativo, Data de pagamento). Revise filtros.")
        #Colocar quais são os ativos duplicados
        ativos_duplicados = df[df.duplicated(subset=["Ativo", "Data de pagamento"], keep=False)]
        ativos_duplicados = ativos_duplicados[["Ativo", "Data de pagamento"]].drop_duplicates()
...
def filtro_generico(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if st.sidebar.checkbox("Filtrar por coluna", key="chk_filter"):
        col = st.sidebar.selectbox("Coluna para filtrar", df.columns.tolist(), key="sel_col")
        valores = sorted(df[col].unique().tolist())
        escolhidos = st.sidebar.multiselect("Valores:", valores, default=valores, key="vals")
...
def plot_relacao_juros(df):
    df_plot = (
        df.groupby(["Ano", "Semestre"], as_index=False)["Juros projetados"].sum()
    ...
    p = (
        ...
        + labs(title="Juros + Amortização Projetados por Ano/Semestre", x="Ano", y="Juros (R$ mil)")
...
def atualizar_session_state_contratos(fundo: str, df_contr: pd.DataFrame):
    ...
    def _salvar():
        if fundo in st.session_state.df_total.columns and not overwrite:
            st.warning("Coluna já existente. Marque 'Sobrescrever' para substituir.")
            return
        if fundo in st.session_state.df_total.columns and overwrite:
            st.session_state.df_total.drop(columns=[fundo], inplace=True)
        st.session_state.df_total = st.session_state.df_total.add(df_contr.rename(columns={"CONTRATOS": fundo}), fill_value=0)
...
def analisar_ativo(df: pd.DataFrame, df_div1: pd.DataFrame):
    ...
    nova_qtd = st.sidebar.number_input(f"Nova quantidade (atual {qtd_atual})", value=qtd_atual, min_value=-100_000)
    if st.sidebar.button("Simular Quantidades Diferentes"):
        df_asset["Quantidade"] = nova_qtd
        df_asset["Juros projetados"] = df_asset["Fluxo descontado (R$)"] * df_asset["Quantidade"]
        df_asset["DIV1_ATIVO"] = df_asset["Juros projetados"] * 0.0001 * (df_asset["Prazos (dias úteis)"] / 252)
...
def analisar_fundo(df: pd.DataFrame, df_div1: pd.DataFrame):
    ...
    ativos_filt = st.sidebar.multiselect("Filtrar ativos do fundo:", ativos_fundo, default=None)
    if not ativos_filt:
    ...
    if ativos_filt:
        ...
        for atv in sorted(df_fundo["Ativo"].unique()):
            q_atual = int(df_fundo.loc[df_fundo["Ativo"] == atv, "Quantidade"].iloc[0])
            novas_qtds[atv] = st.sidebar.number_input(atv, value=q_atual, key=f"num_{atv}")
        if st.sidebar.button("Aplicar quantidades", key="btn_qtd_fundo"):
            ...
            df_fundo["Juros projetados"] = df_fundo["Fluxo descontado (R$)"] * df_fundo["Quantidade"]
            df_fundo["DIV1_ATIVO"] = df_fundo["Juros projetados"] * 0.0001 * (df_fundo["Prazos (dias úteis)"] / 252)
...
def analisar_geral(df: pd.DataFrame, df_div1: pd.DataFrame):
    ...
    fundos_sel = st.sidebar.multiselect("Escolha os fundos para compor a visão:", todos_fundos, default=None)
    if not fundos_sel:
    ...
    ativos_filtr = st.sidebar.multiselect("Filtrar ativos dentro dos fundos selecionados:", ativos_disp, default=None)
    if not ativos_filtr:
